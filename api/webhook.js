  ...
  try {
    ...
    for (const entry of data) {
      ...
      for (const metric of metrics) {
        ...
        logger.info(`Existing dates in table "${tableName}": ${Array.from(existingDates)}`);
...
async function getExistingTables() {
  logger.info('Entering getExistingTables function');
  try {
...
export default async function handler(req, res) {
  logger.info(`Received a ${req.method} request at /api/webhook`);
  if (req.method === 'POST') {
